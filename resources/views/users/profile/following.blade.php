        ...
        @foreach ($user->following as $following)
          <div class="row align-items-center mt-3">
            ...
            <div class="col ps-0 text-truncate">
              <a href="{{ route('profile.show', $following->following->id) }}" class="text-dark fw-bold text-decoration-none">{{ $following->following->name }}</a>
            </div>
              ...
              @if($following->following->id != Auth::user()->id)
                ...
                @else
                  <form action="{{ route('follow.store', $following->following->id) }}" method="post">
                    ...
                    <button type="submit" class="text-primary border-0 bg-transparent p-0 btn-sm">Follow</button>
                  </form>
                @endif
              @endif
            </div>
